        ...
        private void DepositCustomer_Load(object sender, EventArgs e)
        {
            ...
            if (connect.State == ConnectionState.Closed)
            {
                try
                {
                    ...
                    string checkCustomerPassport = "select count(passport_customer) from customersDeposits, customers where customersDeposits.customer_id = customers.id_customer and passport_customer = @passport_customer";
                    ...
                    using (SqlCommand checkCust = new SqlCommand(checkCustomerPassport, connect))
                    {
                        ...
                        else
                        {
                            ...
                            string selectData = "select id_customer_deposit, name_deposit as Вклад, period_dep as 'Период (мес)', deposit_date as 'Дата вклада', return_deposit_date as 'Дата окончания вклада', deposit_amount as 'Сумма вклада' from customersDeposits, customers, deposits where customers.id_customer = customersDeposits.customer_id and deposits.id_deposit = customersDeposits.deposit_id and passport_customer = @passport_customer";
