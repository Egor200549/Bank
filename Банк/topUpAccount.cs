...
namespace Банк
{
    public partial class topUpAccount : Form
    {
        ...
        private void topUpAccount_Load(object sender, EventArgs e)
        {
            ...
            if (connect.State == ConnectionState.Closed)
            {
                try
                {
                    ...
                    string selectData = "select name_deposit, deposit_amount, max_sum, name_currency, current_percentage, bank_account from customersDeposits, deposits, currencies where customersDeposits.deposit_id = deposits.id_deposit and currencies.currency_code = deposits.currency_code and id_customer_deposit = @id_dep;";
                    ...
                    using (SqlCommand cmd = new SqlCommand(selectData, connect))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                ...
                                double sum = reader.GetSqlMoney(1).ToDouble();
                                string max_sum;

                                try
                                {
                                    max_sum = reader.GetSqlMoney(2).ToDouble().ToString("N");
                                }
                                catch
                                {
                                    max_sum = "отсутствует";
                                ...
                                lblNum.Text = "••" + contract_number.Substring(16, 4) + " • Максимальная сумма " + max_sum;
                    ...
                    string checkCustomerPassport = "select count(passport_customer) from customersDeposits, customers, deposits where customersDeposits.customer_id = customers.id_customer and deposits.id_deposit = customersDeposits.deposit_id and passport_customer = @passport_customer and id_customer_deposit != @dep and write_off = 1;";
                    ...
                    using (SqlCommand checkCust = new SqlCommand(checkCustomerPassport, connect))
                    {
                        ...
                        else
                        {
                            ...
                            selectData = "select id_customer_deposit, name_deposit as Название, bank_account as 'Номер счета', deposit_date as 'Дата открытия', return_deposit_date as 'Дата окончания', deposit_amount as 'Сумма', status_dep as 'Статус' from deposits, customersDeposits, customers where write_off = 1 and deposits.id_deposit = customersDeposits.deposit_id and status_dep = 'Действующий' and id_customer_deposit != @dep and customers.id_customer = customersDeposits.customer_id and passport_customer =  @passport";
        ...
        private void btnTranfer_Click(object sender, EventArgs e)
        {
            ...
            if (sum_to_transfer > double.Parse(dataGridView1.SelectedRows[0].Cells[5].Value.ToString()))
            {
